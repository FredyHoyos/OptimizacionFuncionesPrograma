...
class NewtonRaphson:
    ...
    def calcular(self):
        try:
            ...
            dfx_expr2 = sp.diff(dfx_expr, self.x)  # Mantén la derivada simbólica
            fx = float(dfx_expr.subs(self.x, self.xi).evalf())
            dfx = float(dfx_expr2.subs(self.x, self.xi).evalf())
            ...
            # Clasificar si es mínimo o máximo
            tipo_extremo = "mínimo" if dfx > 0 else "máximo"
            self.datos = [("Iter", "x", "f'(x)", "f''(x)", "Error"), f"El {tipo_extremo} se encuentra en x = {self.xi}"]

            return self.resultados, self.iteraciones, self.errores, self.xi, error, self.datos, tipo_extremo
